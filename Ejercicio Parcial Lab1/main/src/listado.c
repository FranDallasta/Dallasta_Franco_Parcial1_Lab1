	...
	do{
		...
		switch(opcion)
		{
		 	...
		 	case 3:
		 		...
		 		break;
		 	case 4:
		 		if(mostrarElectrodomesticoSinReparado(array, LARGO_eletro, arrayReparaciones, LARGO_rep) == -1)
		 		{
		 			printf("\nNo hay electrodomesticos sin reparaciones\n");
		 		}
		 		system("pause");
		 		break;
		 	case 5:
...
int mostrarElectrodomesticoReparado(est_electro aparato[], int LARGO_eletro, est_reparacion arrayReparaciones[], int LARGO_rep, int electroSelec)
{
	...
	{
		for (i=0; i<LARGO_eletro-1; i++)
		{
			if(aparato[i].id_electro == electroSelec && aparato[i].isEmpty != VACIO)
			{
		...
		for (i=0; i<LARGO_eletro-1; i++)
		{
			if(aparato[i].isEmpty != VACIO)
			{
				...
				for (j=0; j<LARGO_rep; j++)
				{
					if(arrayReparaciones[j].isEmpty != VACIO && arrayReparaciones[j].serie_reparacion != aparato[i].serie_electro)
					{
						printf("\nElectrodomestico no reparado num %d ", j);
						printf("ID: %d",aparato[i].id_electro);
						retorno = 0;
					}
				}
			}
